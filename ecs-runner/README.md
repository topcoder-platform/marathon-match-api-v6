...
- Downloaded tester JARs are mode `0400` root-owned files, and serialized scorer config files are mode `0600` root-owned files. Submitted code running as `scorer` cannot read them even if it can guess their `/tmp` paths.
