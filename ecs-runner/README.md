...
- Downloaded tester JARs and serialized scorer config are mode `0600` root-owned files. The child JVM deletes the serialized scorer config immediately after loading it, before tester or submitted solution code executes.
