...
- native `mm-runner-isolate` and `mm-scorer-isolate` helpers that scrub the child JVM environment, run submitted solutions as the separate non-root `scorer` user, restrict submitted-solution filesystem access, and block non-`AF_UNIX` sockets
