...
export class SubmissionRunnerLogService {
  ...
  async getLogsForSubmission(
    ...
    const canReadAnySubmission = this.hasPrivilegedRunnerLogRole(user);
  ...
  /**
   * Checks whether the caller has a role that may read any submission's runner logs.
   * @param user Authenticated caller.
   * @returns True for administrator, copilot, or Manager roles.
   */
  private hasPrivilegedRunnerLogRole(user?: JwtUser): boolean {
    if (!user || !Array.isArray(user.roles)) {
      return false;
    }

    const privilegedRoles = new Set(
      [UserRole.Admin, UserRole.Copilot, UserRole.ProjectManager].map((role) =>
        this.normalizeIdentity(role),
      ),
    );

    return user.roles.some((role) =>
      privilegedRoles.has(this.normalizeIdentity(role)),
    );
  }
